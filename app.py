...
def get_temp_graph(city_list):
    ...
    ax.legend(city_list, fontsize=8, framealpha=0,labelcolor='white')
...
def get_world_graph(city_list):
    ...
    ax.scatter(x=[coord['Lon'] for city, coord in city_coordinates.items()],y=[coord['Lat'] for city, coord in city_coordinates.items()],s=0.1,c=[coord['Lat'] if coord['Lat'] is not None and coord['Lat'] >= 0 else -1 * coord['Lat'] if coord['Lat'] is not None else 0 for city, coord in city_coordinates.items()],alpha=1.0  # Ensure full opacitycmap='hot_r')
    ax.scatter(x=lons,y=lats,s=30,c=cmap[:len(city_list)])
