...
def get_temp_graph(city_list):
    ...
    df.plot.line(ax=ax,marker='o',color=plt.cm.get_cmap('tab10', len(city_list))[:len(city_list)], fontsize=8)
...
def get_world_graph(city_list):
    ...
    ax.scatter(x=lons,y=lats,s=100,c=plt.cm.get_cmap('tab10', len(city_list))[:len(cities)])
